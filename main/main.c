...
/* HTTP & WIFI Code */
esp_err_t http_text_event_handler(esp_http_client_event_t *evt)
{
...
esp_err_t http_image_event_handler(esp_http_client_event_t *evt)
{
	static char *output_buffer;
	static int output_len;
	switch (evt->event_id)
	...
	case HTTP_EVENT_ERROR:
		ESP_LOGI(TAG, "HTTP_EVENT_ERROR");
		lv_label_set_text(http_status_label, "IMAGE HTTP_EVENT_ERROR");
		break;
	case HTTP_EVENT_ON_CONNECTED:
		ESP_LOGI(TAG, "HTTP_EVENT_ON_CONNECTED");
		lv_label_set_text(http_status_label, "IMAGE HTTP_EVENT_ON_CONNECTED");
		break;
	case HTTP_EVENT_HEADER_SENT:
		ESP_LOGI(TAG, "HTTP_EVENT_HEADER_SENT");
		lv_label_set_text(http_status_label, "IMAGE HTTP_EVENT_HEADER_SENT");
		break;
	case HTTP_EVENT_ON_HEADER:
		ESP_LOGI(TAG, "HTTP_EVENT_ON_HEADER, key=%s, value=%s", evt->header_key,
				 evt->header_value);
	...
	case HTTP_EVENT_ON_FINISH:
		...
		if (output_buffer != NULL)
		{
			ESP_LOGI(TAG, "Image size %d", output_len);

			image.data = (uint8_t *)malloc(output_len);
			if (image.data == NULL)
			{
			...
			{
				memcpy(image.data , output_buffer, output_len);
				ESP_LOGI(TAG, "Copied data to image map");

				image.data_size = output_len;// 100 * 100 * 2;

				lv_image_set_src(logo_image, &image);

				ESP_LOGI(TAG, "Shown image");
			}
	...
	case HTTP_EVENT_DISCONNECTED:
		ESP_LOGI(TAG, "HTTP_EVENT_DISCONNECTED");
		lv_label_set_text(http_status_label, "IMAGE HTTP_EVENT_DISCONNECTED");
		if (output_buffer != NULL)
		{
			free(output_buffer);
			output_buffer = NULL;
	...
	case HTTP_EVENT_REDIRECT:
		ESP_LOGI(TAG, "HTTP_EVENT_REDIRECT");
		lv_label_set_text(http_status_label, "IMAGE HTTP_EVENT_REDIRECT");
		break;
	}
	return ESP_OK;
}
...
static void https_with_url(void *pvParameters)
{
	while (true)
	{
		...
		esp_http_client_config_t config = {
			.url = "http://iot.fbiego.com/lvgl/test.txt",
			.event_handler = http_text_event_handler,
			// .crt_bundle_attach = esp_crt_bundle_attach,
			.timeout_ms = 5000,
		};

		lv_label_set_text_fmt(wifi_label, "Text: %s", config.url);
		...
		{
			ESP_LOGE(TAG, "Error with http request: %s", esp_err_to_name(err));
			lv_label_set_text_fmt(response_label, "Error with http request: %s", esp_err_to_name(err));
		}
		...
		esp_http_client_config_t config2 = {
			.url = "http://iot.fbiego.com/lvgl/lvgl-100.bin",
			.event_handler = http_image_event_handler,
			// .crt_bundle_attach = esp_crt_bundle_attach,
			.timeout_ms = 5000,
		};

		lv_label_set_text_fmt(wifi_label, "Image Data: %s", config2.url);
...
void app_main(void)
{
	...
	logo_image = lv_image_create(lv_screen_active());
	lv_image_set_src(logo_image, LV_SYMBOL_OK);
	lv_obj_align(logo_image, LV_ALIGN_TOP_LEFT, 50, 100);
	...
	xTaskCreate(&https_with_url, "https_with_url", 1024 * 10, NULL, 5, NULL);
